...
const app = express(); // ⭐️ Express 앱 객체는 반드시 한 번만 선언합니다.
...
// 서버 시작 시 초기 상품/주문/장바구니 데이터 설정 (미들웨어)
...
// ----------------------------------------------------
// ⭐️ 2. 기본 경로 및 디버깅 라우트 (루트 경로로 변경하여 테스트)
// ----------------------------------------------------

// ⭐️ 디버깅용 임시 POST 라우트 (라우팅 문제를 확인하기 위해 '/' 경로로 설정)
app.post('/', (req, res) => {
    console.log("루트 경로 POST 요청 수신 완료!");
    res.status(200).json({ success: true, message: "루트 테스트 통과" });
});
...
// ⭐️ Vercel 배포를 위한 핵심: Express 앱 객체를 내보냅니다.
module.exports = app;
